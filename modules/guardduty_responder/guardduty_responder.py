...
# Description:
#       Receives event containing GuardDuty finding
#       selectively responds by finding type
#       enriches with NACL ID
#       writes new instanceId-remoteIp pair to DynamoDB
#       updates existing instanceId-repoteIp pair TTL
#       updates NACL rule number in DynamoDB
#       if applicable, writes block rule to applicable NACL
...
#       ttl handling to expire and remove rules
#       max rule number - avoid default allow all
#       better logic for finding type as more are implemented
###############################################################################
